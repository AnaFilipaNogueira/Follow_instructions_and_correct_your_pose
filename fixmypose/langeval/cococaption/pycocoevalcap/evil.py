...
class COCOEvilCap:
    ...
    def evaluate(self, gtscap, rescap, uids, metrics=None, no_metrics=None):
        """
        ...
        if type(gtscap[0]) is list:
            cnt = 0
            print('gts', len(gtscap), 'uuidss', len(uids))
            print('uid', len(uid))
        ...
        for scorer, method in scorers:
            ...
            if type(method) == list:
                for sc, scs, m in zip(score, scores, method):
                    self.setEval(sc, m)
                    self.setImgToEvalImgs(scs, list(gts.keys()), uid, m)
            else:
                self.setEval(score, method)
                self.setImgToEvalImgs(scores, list(gts.keys()), uid, method)
        ...
        if "BLEU" in fetch_metrics or "F1" in fetch_metrics:
            ...
            self.setEval(reduce(lambda x, y: x*y, 
                ...
                1.) ** 0.25, 
                "BLEU")
            self.setImgToEvalImgs(bleus, gt_keys, uid, "BLEU")
        ...
        if "F1" in fetch_metrics:
            ...
            self.setImgToEvalImgs(f1s, gt_keys, uid, "F1")
    ...
    def setImgToEvalImgs(self, scores, imgIds, uid, method):
        ...
        for imgId, score in zip(imgIds, scores):
            ...
            # temp[uid[imgId]] = {"score": score, "gt": self.gts[imgId][0]["caption"], "gen": self.res[imgId][0]["caption"]}
            # temp[uid[imgId]] = {"score": score, "gt": self.gts[imgId][0], "gen": self.res[imgId][0]}
        # with open("eval_" + method + ".json", 'w') as f:
