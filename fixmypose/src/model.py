...
class Encoder(nn.Module):
    def __init__(self, feature_size, normalized=False):
        ...
        if args.alteracoes == 'normal_PTFalse':
            resnet_extractor = models.resnet101(pretrained=False)
        else:
    ...
    def forward(self, src, trg):
        ...
        if args.img_type == 'feat':
            ...
            # Feature Extraction
            src_feat = self.resnet_extractor(src.float())
            trg_feat = self.resnet_extractor(trg.float())
