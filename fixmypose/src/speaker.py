...
class Speaker:
    def __init__(self, dataset):
        ...
        self.encoder = Encoder(self.feature_size).cuda() #extrai as carateristicas
        ctx_size = self.encoder.ctx_dim #tamanho das carateristicas
        ...
        wandb.log({"softmax_loss": self.softmax_loss})
        ...
        for epoch in range(num_epochs):
            ...
            for i, (uid, src, trg, inst, leng) in iterator:
                ...
                if rl:
                    ...
                    print('\nINST', inst)
                    print('\nLENG', leng)
                    loss, batch_word_accu = self.rl_training(uid, src, trg, inst, leng, reward_func)
                else:
            ...
            wandb.log({"word_accuracy": word_accu, "train_loss": loss})
            ...
            if epoch % 1 == 0:
                ...
                for key in sorted(scores.keys()):
                    ...
                    wandb.log({"{}".format(key): scores[key]})
                ...
                print(log_str)
                wandb.log({"main_score": main_score, "minor_score": minor_score})
    ...
    def teacher_forcing(self, src, trg, inst, leng, train=True):
        ...
        #print(np.shape(src), np.shape(trg))
        #print(src)
    ...
    def infer_batch(self, src, trg, sampling=True, train=False):
        ...
        h_t = torch.zeros(1, batch_size, args.hid_dim).cuda()
        c_t = torch.zeros(1, batch_size, args.hid_dim).cuda()
        ended = np.zeros(batch_size, bool)
    ...
    def load_init(self, path):
        ...
        pretrained_enc_dict = {k: v for k, v in enc_state_dict.items() if k in encoder_dict}
        pretrained_dec_dict = {k: v for k, v in dec_state_dict.items() if k in decoder_dict}
    ...
    def load(self, path):
        print("Load Speaker from %s" % path)
        enc_path = os.path.join(path + "/best_eval" + "_enc.pth")
        dec_path = os.path.join(path + "/best_eval" + "_dec.pth")
        enc_state_dict = torch.load(enc_path)
        dec_state_dict = torch.load(dec_path)
