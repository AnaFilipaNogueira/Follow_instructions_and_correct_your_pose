...
class Tokenizer:
    ...
    def encode(self, sent): #pega na frase divide nas suas palavras e retorna os indices correspondentes de cada palavra
        words = self.tokenize(sent)
        return list(map(lambda word: self.word2idx(word), words))

    def decode(self, idx): #pega nos indices e vai buscar as palavras correspondentes e junta as diversas palavras adicionando um " " entre elas
