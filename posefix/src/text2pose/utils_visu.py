...
mva = MeshViewer(width=imw, height=imh, use_offscreen=True)
mvb = MeshViewer(width=imw, height=imh, use_offscreen=True)
...
def image_from_body_vertices(body_vertices, faces, viewpoints=[[]], color='grey', add_ground_plane=False, two_views=0):
	...
	for vp in viewpoints:
		...
		save_imgs = Image.fromarray(body_image) 
		save_imgs.save('/home/up201705948/posescript-main/images_saved/img_hope.jpg')

	return imgs


def image_from_pair_data(path_img0, path_img1, pose_A_data, pose_B_data, body_model, viewpoint=[], pose_a_color='grey', pose_b_color='purple', add_ground_plane=False):
	"""
	...
	mva.scene.set_pose(mva.camera_node, pose=camera_pose)
	mvb.scene.set_pose(mvb.camera_node, pose=camera_pose)

	# arange meshes
	#mv.set_static_meshes(([ground_plane] if add_ground_plane else []) + [body_mesh_a, body_mesh_b])
	mva.set_static_meshes(([ground_plane] if add_ground_plane else []) + [body_mesh_a])
	mvb.set_static_meshes(([ground_plane] if add_ground_plane else []) + [body_mesh_b])
	
	# produce the image
	image_a = mva.render(render_wireframe=False)
	image_a = np.array(Image.fromarray(image_a)) # img of shape (H, W, 3)
	save_imgs_a = Image.fromarray(image_a) 
	save_imgs_a.save(path_img0)
	
	image_b = mvb.render(render_wireframe=False)
	image_b = np.array(Image.fromarray(image_b)) # img of shape (H, W, 3)
	save_imgs_b = Image.fromarray(image_b) 
	save_imgs_b.save(path_img1)
